...
public class RepositoryTes  {
    ...
    public List<RestResult> getAll() {
        System.out.println("Mengambil data absensi" + jdbcTemplate1.getServerTime());
        String sql = "SELECT "
                + " a.id_absensi AS id,"
    ...
    public RestResult getRestById(int id) {
        String sql2 = "SELECT a.id_absensi AS id, ang.nama_lengkap AS nama, ang.nim AS key, " +
                "r.nama_ruangan AS value, a.id_ruangan AS rand, a.jam_masuk AS waktu_input " +
                "FROM absensi_divisi a " +
                "JOIN anggota ang ON a.nim_anggota = ang.nim " +
                "JOIN ruangan r ON a.id_ruangan = r.id_ruangan " +
                "WHERE a.id_absensi = ?";
        RestResult restexamplecrud2 = jdbcTemplate1.queryForObject(sql2, new Object[]{id}, new RestMapper());
    ...
    public void updateRest(Rest restexamplecrud, int id) throws SQLException {
        ...
        // Update nama in anggota table
        String sqlNama = "UPDATE anggota SET nama_lengkap=? WHERE nim=?";
        Object[] paramNama = new Object[] {
            restexamplecrud.getNama(),
            restexamplecrud.getKey()
        };
    ...
    public RestResult handleAbsensi(Rest rest) throws SQLException {
        ...
        if (ids.isEmpty()) {
            // No open absensi, insert new (masuk)
            ...
            // Open absensi found, update jam_keluar (keluar)
